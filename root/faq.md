...
2. 运行 `npm install`
3. 运行 `npm start`
4. 默认会跑起来我们的 demo 例子，demo 代码位置: [examples/starter-bot.js](https://github.com/Chatie/wechaty-getting-started/blob/master/examples/starter-bot.js)
5. 修改 demo 的例子，实现你自己想要的 bot 逻辑

> 若有兴趣测试 demo 中的其他案例，请在 package.json 文件中 scripts 更改脚本启动命令。
  ```javascript
  "start": "node examples/advanced/gist-bot/index.js"
  ```
...
### 2.2. wechaty 是支持个人号还是公众号？    <a id="personal-vs-official"></a>
...
### 2.3. 如何获取到用户的微信号    <a id="how-to-get-id"></a>
...
* 如果使用的是 Web 版本的 Puppet，Puppeteer 版本由于网页微信的机制，大部分情况下获取不到微信号，只有一小部分用户能获取到微信号。
* 如果使用的是 iPad 版本的 Puppet，PadPro 版本是可以拿到微信号的，但是前提这个 Contact 需要和 bot 是好友关系，非好友关系是拿不到微信号的。
...
### 2.4. 如何获取好友和群的唯一id？    <a id="unique-id"></a>

通过 `Contact.id` 和 `Room.id` 获取好友和群的唯一 id。

不同的 Puppet，这个唯一 id 是不同的。
...
* 基于 iPad 版本的 Puppet 得到的 id 跨 session 是不会变化的，可以用来当做这个联系人的唯一标识码。
...
* [\#1276](https://github.com/Chatie/wechaty/issues/1276) 最新版本 Contact 没有 uin 字段了吗？
...
> 是否可以通过 wechaty 发送分享链接？

PuppetPadchat, PuppetPadpro 是支持的， 其他版本是不支持的，示例代码：
...
### 2.6. 机器人发消息有几种方法？    <a id="why-not-send-with-on-message"></a>

**1. 通过 message 直接回复消息（疯狂回复，有消息就回）**

```typescript
import { Wechaty } from 'wechaty'
const bot = new Wechaty()
bot.on('message', async msg => {
  await msg.say('receive your message')
})
```

**2. 通过 room 在群内发送消息（只回复群消息，可以通过自己的设置，限定特定群的回复）**
...
**3. 通过 contact 给一个好友发消息（将接受到的文本消息转发给特定的联系人）**
...
### 2.7. 机器人被拉到一个新的群组里的事件是否支持？    <a id="room-join-support"></a>
...
bot.on('room-join', async (room, inviteeList, inviter) => {
  ...
  // 如果机器人被拉到一个新的群组里, inviteeList[0] === bot.self() [ Symbol(You) ]
})
```


### 2.8. 通过 Room.find() 找到的群，要是用户名或群名字有相同的会怎么办，find找到的是哪一个？    <a id="room-find-same-topic"></a>

返回找到的第一个群，排序方式是随机的。

### 2.9. 如何发送一个链接邀请好友进群？    <a id="invite-friend-to-room"></a>
...
### 2.10. 为什么我通过Room.findAll只拿到了一部分的群    <a id="room-list-not-complete"></a>
...
### 2.11. 我的群好多，我等不了ready事件就想要操作bot的群    <a id="too-many-rooms-to-wait"></a>

这个问题暂时没法完全解决，有两个临时的解决方案，首先要明确一下需求：

**我需要操作的这个群是我的长期管理的群，每次启动都需要拿到这个群**

对于这种情况，可以在开发阶段先拿到这个群的 `id` 并且保存这个 `id`，之后每次启动之后，主动加载这个 room。

```javascript
import { Wechaty } from 'wechaty'
const bot = new Wechaty()
bot.on('login', async user => {
    const room = await bot.Room.load(roomId)
    await room.sync()
...
对于这种情况，群事件（`room-join`, `room-leave`, `room-topic`, `room-invite`）在触发了之后，都会对当前这个群做一次加载，所有监听到的事件里面，都是可以直接拿到这个群的信息的，是不需要等待的，还有当群里面有消息的时候，如果当前的群并没有被加载过，也是会手动加载这个群的，然后可以通过消息对象来获取这个群。
...
### 2.12. 为什么我的bot疯狂的在说话    <a id="endless-talking"></a>
...
### 2.13. 重新登陆之后是否可以读到之前的消息    <a id="endless-talking"></a>
...
### 3.2. 再次运行时，如何不用扫码就能登陆机器人    <a id="login-status-persistent"></a>

在启动的时候可以设置 wechaty 的 name（旧版本叫 profile ） 来存储机器人的登陆信息，登陆后会自动生成一个 `*.memory-card.json` 的文件，这样再次运行的时候，就不需要扫码就可以直接登陆机器人了

```typescript
import { Wechaty } from 'wechaty'
const bot = new Wechaty({ name: 'your-cute-bot-name'})
```
...
如果你想在一个机器上启动多个机器人，你可以通过设置 `name`（旧版本叫 profile ） 的值设置不同的机器人登陆信息，对应的你得到多个 `*.memory-card.json` 文件。有两种方法可以设置 profile

**1. wechaty 初始化的时候设置 name**
...
### 3.5. wechaty 运行的 bot 如何修改文件后自动重启    <a id="hot-reload"></a>
...
1. 有一些 Puppet 是不支持这个功能的，比如使用网页 API 的 PuppetWechat4u
...
> 这么理解：wechat4u 有的，wechaty 都有，反之不一定有，对么？
...
{% hint style="info" %}
如果 FAQ 的内容无法满足你，可以去官网发Issue 获取支持： [https://github.com/chatie/wechaty/issues](https://github.com/chatie/wechaty/issues)
{% endhint %}
